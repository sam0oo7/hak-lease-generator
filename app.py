...
with st.form("lease_form"):
    ...
    with st.expander("👥 Tenant Details"):
        tenant_name           = st.text_input("Tenant Name", "Zahida Noman Patel")
        tenant_company        = st.text_input("Tenant Company", "Apex Global Hub Ltd.")
        tenant_address        = st.text_input("Tenant Address", "59 Manor Dr, Wembley HA9 8EB")
        tenant_company_number = st.text_input("Tenant Company No.", "16396293")
        tenant_signatory      = st.text_input("Tenant Signatory", "Zahida Noman Patel")
        tenant_phone          = st.text_input("Tenant Phone", "07459226661")
        tenant_notice_address = st.text_input("Tenant Notice Address", tenant_address)

    with st.expander("💷 Financials & Use"):
        base_rent             = st.number_input("Base Rent (£/mo)", 0, 10_000, 1_000, step=50)
        base_rent_note        = st.text_input("Landlord’s Note (on Base Rent)", "£1,250/m from 2nd year onwards")
        permitted_use_address = st.text_input("Permitted Use Address", "Unit 2, 81 High St, NW10 4NT")
        permitted_use         = st.text_input("Permitted Use", "Money Exchange / Other Services")
